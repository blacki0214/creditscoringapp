...
class FirebaseAuthService {
  ...
  Future<bool> checkEmailExists(String email) async {
    try {
      ...
      final signInMethods = await _firebase.auth.fetchSignInMethodsForEmail(
        email,
      );
  ...
  Future<Map<String, dynamic>> checkEmailForPasswordReset(String email) async {
    try {
      ...
      if (currentUser != null &&
        ...
        print(
          '[DEBUG] Current user providers: ${providerData.map((p) => p.providerId).toList()}',
        );
        ...
        print(
          '[DEBUG] hasPassword: $hasPassword, hasGoogle: $hasGoogle, hasPhone: $hasPhone',
        );

        // PRIORITY: If account has password, allow reset (even if it also has Google/Phone)
        if (hasPassword) {
          return {'canReset': true, 'message': 'OK'};
        }
        ...
        if (hasGoogle) {
          return {
            'canReset': false,
            'message':
                'This account uses Google Sign-In. Please sign in with Google instead of resetting password.',
          };
        }
        ...
        if (hasPhone) {
          return {
            'canReset': false,
            'message':
                'This account uses phone authentication. Please sign in with your phone number.',
          };
        }
        ...
        return {
          'canReset': false,
          'message':
              'This account does not use password authentication. Please use your original sign-in method.',
        };
      }
      ...
      if (e.code == 'invalid-email') {
        return {'canReset': false, 'message': 'Invalid email address format.'};
      }
      throw _handleAuthException(e);
    } catch (e) {
      throw Exception('Failed to check email: $e');
  ...
  Future<void> linkPasswordToAccount(String password) async {
    ...
    try {
      ...
      print(
        '[DEBUG] FirebaseAuthException during linking: ${e.code} - ${e.message}',
      );
      throw _handleAuthException(e);
    } catch (e) {
